...
class Pipeline(BaseModel):
    ...
    models_to_train: dict = None
    acoustic_features_to_extract: list = None
    ...
    def _logger_setup(self):
        if self.app_logger is None:
            try:
                ...
                self.app_logger.warning("Pipeline - No logger was passed. Using a default one.")
    ...
    def _load_config_from_a_json(self) -> dict:
        ...
        try:
            ...
            if json_as_dict == {}:
                raise ValueError("Pipeline - The specified file is empty or has an empty dictionary.")

        except FileNotFoundError as e:
            raise FileNotFoundError(f"Pipeline - File '{str_with_path}' does not exist.") from e
        except json.JSONDecodeError as e:
            raise ValueError(f"Pipeline - An error occurred while decoding the JSON file: {e}") from e
        except Exception as e:
            raise ValueError(f"Pipeline - An error occurred while reading the JSON file: {e}") from e
    ...
    def setup_using_config(self):
        ...
        try:
            ...
            self.app_logger.error(f"Pipline - Error loading the config file: {self.config_file_path}")
            raise IOError(e)
    ...
    def run_an_experiment_from_config(self):
        try:
            ...
            config_model_experiment = self.configurations_as_dict.get("model")
            ...
            self.app_logger.error("Pipeline - Error setting up the experiment.")
            raise RuntimeError(e)

    def run_pipeline(self, function_to_apply_over_metadata: callable):
        self.setup_using_config()
        self.process_dataset(function_to_apply_over_metadata=function_to_apply_over_metadata)
