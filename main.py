...
def make_dicoperia_metadata_cougth_after(
        metadata: pd.DataFrame, filters_: dict = None, remove_samples: dict = None
):
    """
    Make a metadata file for the COPERIA dataset filtering some columns
...
def make_dicoperia_metadata_cougth_before(
        metadata: pd.DataFrame, filters_: dict = None, remove_samples: dict = None
):
    """
    Make a metadata file for the COPERIA dataset filtering some columns
...
def make_dicoperia_metadata_a_after(
        metadata: pd.DataFrame, filters_: dict = None, remove_samples: dict = None
):
    """
    Make a metadata file for the COPERIA dataset filtering some columns
...
def make_dicoperia_metadata_a_before(
        metadata: pd.DataFrame, filters_: dict = None, remove_samples: dict = None
):
    """
    Make a metadata file for the COPERIA dataset filtering some columns
...
if __name__ == "__main__":
    ...
    pipeline_app.setup_using_config()

    for model in ["LogisticRegression", "RandomForest", "MLP"]:
        pipeline_app.configurations_as_dict["model"]["name"] = model
        ...
        new_pipeline_config["dataset"][
            "path_to_object"] = "/home/visia/Documents/GitHub/coperia_sci/exp/COPERIA-AFTER.pkl"
        ...
        new_pipeline_config["dataset"][
            "path_to_object"] = "/home/visia/Documents/GitHub/coperia_sci/exp/COPERIA-BEFORE.pkl"
        ...
        new_pipeline_config["dataset"][
            "path_to_object"] = "/home/visia/Documents/GitHub/coperia_sci/exp/COPERIA-COUGH.pkl"
        ...
        # Run the pipeline with all the data a and default configuration
        new_pipeline_config = pipeline_app.configurations_as_dict
        new_pipeline_config["run"]["run_name"] = "COPERIA-A-AFTER"
        new_pipeline_config["dataset"][
            "path_to_object"] = "/home/visia/Documents/GitHub/coperia_sci/exp/COPERIA-A-AFTER.pkl"
        new_pipeline_config["dataset"]["filters"] = {"audio_type": ["/a/"], "audio_moment": ["after"]}
        pipeline_app.configurations_as_dict = new_pipeline_config
        pipeline_app.run_pipeline_with_an_experiment_from_config(
            make_dicoperia_metadata_a_after
        )

        # Run the pipeline with all the data a and default configuration
        new_pipeline_config = pipeline_app.configurations_as_dict
        new_pipeline_config["run"]["run_name"] = "COPERIA-A-BEFORE"
        new_pipeline_config["dataset"][
            "path_to_object"] = "/home/visia/Documents/GitHub/coperia_sci/exp/COPERIA-A-BEFORE.pkl"
        new_pipeline_config["dataset"]["filters"] = {"audio_type": ["/a/"], "audio_moment": ["before"]}
        pipeline_app.configurations_as_dict = new_pipeline_config
        pipeline_app.run_pipeline_with_an_experiment_from_config(
            make_dicoperia_metadata_a_before
        )

        # Run the pipeline with all the data a and default configuration
        new_pipeline_config = pipeline_app.configurations_as_dict
        new_pipeline_config["run"]["run_name"] = "COPERIA-COUGH-AFTER"
        new_pipeline_config["dataset"][
            "path_to_object"] = "/home/visia/Documents/GitHub/coperia_sci/exp/COPERIA-COUGH-AFTER.pkl"
        new_pipeline_config["dataset"]["filters"] = {"audio_type": ["/cough/"], "audio_moment": ["after"]}
        pipeline_app.configurations_as_dict = new_pipeline_config
        pipeline_app.run_pipeline_with_an_experiment_from_config(
            make_dicoperia_metadata_cougth_after
        )

        # Run the pipeline with all the data a and default configuration
        new_pipeline_config = pipeline_app.configurations_as_dict
        new_pipeline_config["run"]["run_name"] = "COPERIA-COUGH-BEFORE"
        new_pipeline_config["dataset"][
            "path_to_object"] = "/home/visia/Documents/GitHub/coperia_sci/exp/COPERIA-COUGH-BEFORE.pkl"
        new_pipeline_config["dataset"]["filters"] = {"audio_type": ["/cough/"], "audio_moment": ["before"]}
        pipeline_app.configurations_as_dict = new_pipeline_config
        pipeline_app.run_pipeline_with_an_experiment_from_config(
            make_dicoperia_metadata_cougth_before
        )
